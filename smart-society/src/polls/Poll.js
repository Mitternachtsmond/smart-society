      ...
      <div className="flex justify-between font-semibold text-gray-800">
        ...
        <div className="md:col-span-2 text-right md:row-start-5">
          <div className="flex flex-row justify-between">
            <button
              className="bg-red-400 hover:bg-red-600 text-white font-bold my-3 py-2 px-4 rounded float-right mr-10"
              onClick={() => props.handler(props.id)}
            >
              End Poll
            </button>
            <Link to={`/polls/vote/${props.id}`}>
              <button className="bg-green-400 hover:bg-green-600 text-white font-bold my-3 py-2 px-4 rounded float-right">
                Vote
              </button>
            </Link>
          </div>
